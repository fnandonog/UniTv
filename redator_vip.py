...
# ==========================================
# 2. TEMPLATES BASE (ATUALIZADOS COM FOOTER E CHATBOT VIP)
# ==========================================
...
<head>
    ...
    <link rel="canonical" href="https://unitvsite.com.br/blog/" />
      ...
      "@graph": [
        {
          ...
          "author": { "@type": "Organization", "name": "Equipe UniTV VIP" },
          "publisher": { "@type": "Organization", "name": "UniTV Oficial Brasil", "logo": { "@type": "ImageObject", "url": "https://unitvsite.com.br/img/logo-unitv.png" } },
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; font-family: 'Poppins', sans-serif; }
        :root { --bg-body: #050505; --bg-card: #0a0a0a; --neon-azul: #00f3ff; --neon-laranja: #ff7300; --gradiente-premium: linear-gradient(135deg, #FF512F 0%, #DD2476 100%); --borda-sutil: 1px solid rgba(255, 255, 255, 0.08); --wa-header: #075e54; --wa-bg: #e5ddd5; --wa-msg-user: #dcf8c6; --cor-destaque: #ff0080; }
        ...
        header.scrolled { top: 0; height: 70px; background: rgba(5, 5, 5, 0.98); border-bottom: 1px solid rgba(0, 243, 255, 0.2); }
        ...
        .article-hero { padding: 160px 5% 60px; text-align: center; background: radial-gradient(circle at 50% 20%, #2d0b1e 0%, #050505 60%); margin-top: 38px; }
        .article-hero h1 { font-family: 'Montserrat', sans-serif; font-size: clamp(2rem, 5vw, 3.5rem); line-height: 1.1; font-weight: 900; margin-bottom: 20px; color: #fff;}
        .post-meta { font-size: 0.85rem; color: #777; margin-bottom: 30px; text-transform: uppercase; letter-spacing: 2px; font-weight: 600; }
        .badge { background: rgba(255, 0, 85, 0.1); color: #ff0055; padding: 6px 15px; border-radius: 50px; font-size: 0.75rem; font-weight: 800; display: inline-block; margin-bottom: 20px; text-transform: uppercase; letter-spacing: 1px; }

        .article-content { max-width: 900px; margin: 0 auto; padding: 40px 20px; }
        .article-text { font-size: 1.15rem; color: #ccc; line-height: 1.8;}
        .article-text h2 { color: #fff; margin: 50px 0 20px; font-size: 2rem; border-left: 4px solid var(--cor-destaque); padding-left: 15px; font-weight: 800;}
        .article-text h3 { color: #ddd; margin: 30px 0 15px; font-size: 1.5rem; font-weight: 700; }
        .article-text p { margin-bottom: 25px; }
        .article-text strong { color: #fff; font-weight: 700; }
        ...
        .tech-box { background: var(--bg-card); border: 1px solid rgba(255,255,255,0.05); border-left: 4px solid #00ff88; padding: 25px; margin: 35px 0; border-radius: 8px; }
        .tech-box h4 { color: #00ff88; margin-bottom: 10px; font-size: 1.2rem; text-transform: uppercase; letter-spacing: 1px; }
        .tech-box p { margin-bottom: 0; font-size: 1rem; color: #ddd; }
        
        .article-text blockquote { background: rgba(255, 115, 0, 0.05); border-left: 4px solid var(--neon-laranja); padding: 25px; margin: 30px 0; font-size: 1.2rem; font-style: italic; border-radius: 0 12px 12px 0; color: #eee; }
        ...
        .about-text h2 { font-size: 2.8rem; font-weight: 900; line-height: 1.1; margin-bottom: 20px; color: #fff; border:none; padding:0; } .about-text h2 span { background: var(--gradiente-premium); -webkit-background-clip: text; -webkit-text-fill-color: transparent; }
        ...
        @media (max-width: 992px) { .about-grid { grid-template-columns: 1fr; text-align: center; } .about-features li { justify-content: center; } }
        @media (max-width: 768px) { header { padding: 10px 20px; } .nav-links { display: none; } .article-hero h1 { font-size: 2.2rem; } .footer-container { grid-template-columns: 1fr; text-align: center; } .footer-links a { justify-content: center; } .payment-methods { justify-content: center; } .wa-widget { width: calc(100% - 40px); left: 20px; right: 20px; bottom: 95px; max-height: 75vh; } }
    </style>
</head>
<body>
    ...
    <header id="main-header">
        ...
        <nav class="nav-links">
            ...
            <a href="../index.html">Blog Tech</a>
            <a href="../../cinema/index.html">Catálogo VIP</a>
        </nav>
        ...
        <div class="footer-container">
            ...
            <div class="footer-col">
                ...
                <ul class="footer-links">
                    ...
                    <li><a href="../../cinema/index.html"><i class="fa-solid fa-angle-right"></i> Cinema VIP</a></li>
                </ul>
            </div>
            <div class="footer-col">
...
# ==========================================
# 4. INJEÇÃO SEGURA NA VITRINE (A VACINA DA CLONAGEM)
# ==========================================
def atualizar_pagina_principal_do_blog(titulo, slug, meta_desc):
    caminho = os.path.join(PASTA_BLOG, "index.html")
    # AQUI ESTÁ A VACINA: Ele vai procurar esta tag exata para colar o post em baixo.
    alvo = '<main class="blog-container" id="blogCatalog">'
    ...
    try:
        ...
        img_ia = f"https://image.pollinations.ai/prompt/cyberpunk%20tech%20dark%20background%20vibrant%20blue%20orange%20neon%20futuristic%20{url_slug}?width=800&height=500&nologo=true"
        
        card = f"""
        <a href="{slug}/index.html" class="post-card reveal active" style="text-decoration:none; display: flex; flex-direction: column;">
            <div class="post-thumb" style="width: 100%; height: 220px; background: #111; overflow: hidden; position: relative; border-radius: 10px; margin-bottom: 15px;">
                <img src="{img_ia}" alt="{titulo}" style="width: 100%; height: 100%; object-fit: cover; transition: 0.6s;">
            </div>
            <div class="post-content" style="padding: 0; display: flex; flex-direction: column; flex-grow: 1;">
                <span class="post-tag" style="background: rgba(255, 0, 85, 0.1); color: #ff0055; font-size: 0.65rem; font-weight: 800; padding: 6px 15px; border-radius: 50px; text-transform: uppercase; margin-bottom: 15px; display: inline-block; width: fit-content;">NOVIDADE TECH</span>
                <h2 style="font-size: 1.4rem; margin-bottom: 15px; color: #fff; font-weight: 800; line-height: 1.2;">{titulo}</h2>
                <p style="color: #777; font-size: 0.9rem; margin-bottom: 25px; line-height: 1.6; flex-grow: 1;">{meta_desc}</p>
                <span class="btn-read" style="color: #ff0080; text-decoration: none; font-weight: 800; font-size: 0.85rem; display: flex; align-items: center; gap: 10px; text-transform: uppercase; margin-top: auto;">LER GUIA <i class="fa-solid fa-arrow-right"></i></span>
            </div>
        </a>"""

        if alvo in html:
            # Cola a imagem do post blindado UMA ÚNICA VEZ
            html = html.replace(alvo, alvo + "\n" + card)
            with open(caminho, "w", encoding="utf-8") as f: 
                f.write(html)
            print(f"🔗 Post '{titulo}' colado na vitrine do Blog com sucesso.")
        else:
            print("⚠️ ERRO CRÍTICO: Tag alvo '<main class=\"blog-container\" id=\"blogCatalog\">' não encontrada no index do blog!")
    except Exception as e:
...
# EXECUTOR
# ==========================================
print("🚀 MÁQUINA DE SEO UniTV (VERSÃO BLINDADA E PREMIUM)")
